...
router.post("/createURL", async (req, res) => {
  try {
    ...
    // Check for existing ID (uncomment if desired)
    // const existingURL = await getURL({ urlID: id });
    // if (existingURL) {
    ...
    const data = { ...req.body, shortURL, urlID: id, clicked: 0 };
    const result = await addURL(data);

    if (!result.acknowledged) {
      return res.status(400).json({ message: "Error adding URL" }); 
    }
